...
def test_diff_nested_json():
    data1 = read_data('tests/fixtures/input_json/nested_data1.json')
    data2 = read_data('tests/fixtures/input_json/nested_data2.json')
    expected = read_data('tests/fixtures/expected/compare_nested.json')['diff']
    result = compare(data1, data2)
    assert(result == expected)


def test_diff_nested_json_reversed():
    data1 = read_data('tests/fixtures/input_json/nested_data1.json')
    data2 = read_data('tests/fixtures/input_json/nested_data2.json')
    expected = read_data(
